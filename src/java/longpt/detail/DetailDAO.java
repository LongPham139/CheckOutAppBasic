...
public class DetailDAO {
    ...
    public String getLastDetail() throws NamingException, SQLException {
        ...
        try {
            ...
            String sql = "SELECT TOP 1 detailID, stt FROM tblOrderDetail ORDER BY stt DESC";
    ...
    public boolean insertDetail(DetailDTO detail) throws SQLException, NamingException {
        ...
        try {
            con = DBUtils.getConnection();
            String sql = "INSERT INTO tblOrderDetail(detailID, productID, quantity, oderID, stt) "
                    + "VALUES(?,?,?,?,?)";
            ps = con.prepareStatement(sql);
            ps.setString(1, detail.getDetailID());
            ps.setString(2, detail.getProductID());
            ps.setInt(3, detail.getQuantity());
            ps.setString(4, detail.getOderID());
    ...
    public int countUp() throws NamingException, SQLException {
        ...
        try {
            con = DBUtils.getConnection();
            String sql = "SELECT TOP 1 stt FROM tblOrderDetail ORDER BY stt DESC";
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            if(rs.next()){
                count = rs.getInt("stt");
